...
import { Home, Search, Compass, MessageCircle, Heart, PlusSquare, Menu, User, Play, Pause, Music, Volume2, VolumeX } from 'lucide-react';
...
function NewFeed() {
  ...
  const fetchCurrentUser = useCallback(async () => {
    try {
        ...
        // Ví dụ: localStorage.setItem('username', response.username);
        ...
        // Giả sử axiosClient đã config base url là /api/v1
  ...
  return (
    <div className="flex min-h-screen bg-white">
      ...
      <aside className="fixed left-0 top-0 h-screen w-[245px] border-r border-gray-300 bg-white px-3 py-8 flex flex-col">
        <div className="mb-10 px-3"><h1 className="text-2xl font-semibold" style={{ fontFamily: 'cursive' }}>BQMUSIC</h1></div>
        <nav className="flex-1 space-y-2">
            ...
            <div onClick={() => setIsCreateModalOpen(true)} className="flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-100 transition cursor-pointer">
                <PlusSquare className="w-6 h-6"/> <span className="font-medium">Tạo</span>
            </div>
            <NavItem icon={<User className="w-6 h-6"/>} label="Trang cá nhân" />
        </nav>
        <a href="#" className="flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-100 transition mt-auto"><Menu className="w-6 h-6" /> <span className="font-medium">Xem thêm</span></a>
      </aside>
      ...
      <aside className="fixed right-0 top-0 h-screen w-[320px] bg-white px-8 py-8 overflow-y-auto hidden lg:block">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-4"><span className="text-gray-500 font-semibold text-sm">Gợi ý cho bạn</span><button className="text-xs font-semibold">Xem tất cả</button></div>
          <div className="space-y-3">
            {suggestions.map(user => (
              <div key={user.id} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <img src={user.avatar} alt={user.username} className="w-8 h-8 rounded-full object-cover" onError={(e) => {e.target.src = DEFAULT_AVATAR_URL}} />
                  <div><div className="font-semibold text-sm">{user.username}</div><div className="text-gray-500 text-xs">{user.mutual}</div></div>
                </div>
                <button onClick={() => handleFollow(user.id)} className={`text-xs font-semibold px-2 py-1 rounded transition-colors ${user.isFollowed ? 'text-red-500 hover:text-red-700 bg-red-50' : 'text-blue-500 hover:text-blue-700'}`}>{user.isFollowed ? "Hủy theo dõi" : "Theo dõi"}</button>
              </div>
            ))}
          </div>
        </div>
        <div className="mt-8 text-xs text-gray-400 space-y-2"><div>© 2024 INSTAGRAM FROM META</div></div>
      </aside>
    </div>
  );
}
...
// Helpers
const NavItem = ({ icon, label }) => (<a href="#" className="flex items-center gap-4 px-3 py-3 rounded-lg hover:bg-gray-100 transition">{icon}<span className="font-medium">{label}</span></a>);
